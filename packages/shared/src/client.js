...
import { FreeeError } from '@mcp-server/types';
...
export class FreeeClient {
    ...
    async getDeals(params) {
        ...
        if (params.start_issue_date)
            searchParams.append('start_issue_date', params.start_issue_date);
        if (params.end_issue_date)
            searchParams.append('end_issue_date', params.end_issue_date);
        if (params.limit)
            searchParams.append('limit', params.limit.toString());
        if (params.offset)
            searchParams.append('offset', params.offset.toString());
        return this.get(`/api/1/deals?${searchParams.toString()}`);
    }
    async getAccountItems(companyId) {
        return this.get(`/api/1/account_items?company_id=${companyId}`);
    }
    async getPartners(companyId) {
        return this.get(`/api/1/partners?company_id=${companyId}`);
    }
    async getSections(companyId) {
        return this.get(`/api/1/sections?company_id=${companyId}`);
    }
    async getItems(companyId) {
        return this.get(`/api/1/items?company_id=${companyId}`);
    }
    async getTags(companyId) {
        return this.get(`/api/1/tags?company_id=${companyId}`);
    }
    async getTrialBalance(params) {
        const searchParams = new URLSearchParams();
        searchParams.append('company_id', params.company_id.toString());
        if (params.fiscal_year)
            searchParams.append('fiscal_year', params.fiscal_year.toString());
        if (params.start_month)
            searchParams.append('start_month', params.start_month.toString());
        if (params.end_month)
            searchParams.append('end_month', params.end_month.toString());
        return this.get(`/api/1/trial_balance?${searchParams.toString()}`);
    }
