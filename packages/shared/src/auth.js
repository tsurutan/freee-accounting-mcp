...
export class FreeeOAuthClient {
     ...
     * 認証URLを生成
     */
    generateAuthUrl(state, enableCompanySelection = true) {
    ...
    async exchangeCodeForTokens(code) {
        try {
            ...
            const response = await this.httpClient.post('/public_api/token', params.toString(), {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
            });
            ...
            throw new FreeeError(errorMessage, error.response?.status || 500, error.response?.data?.errors, error, error.response?.headers?.['x-request-id']);
        }
    }
    /**
     ...
     * 詳細なエラーメッセージを生成
     */
    getDetailedErrorMessage(error, defaultMessage) {
        if (!error.response) {
        ...
        if (data?.errors && Array.isArray(data.errors)) {
            const errorMessages = data.errors.map((err) => err.message || err.code).join(', ');
